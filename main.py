...
def run_beast2_simulations_parallel(simulation_xml_list, num_jobs, beast_seed_str, max_retries=3):
    # Validate that the seed is a valid integer
    try:
        base_seed = int(beast_seed_str)
    except ValueError:
        raise ValueError(f"beast_seed_str must be a valid integer, got: {beast_seed_str}")

    def cleanup_intermediate_files(simulation_xml, include_time=True):
        """
        Remove intermediate files generated by a failed simulation.

        This cleans up .tree and .traj files to ensure a fresh
        state before retrying the simulation. Optionally includes
        the .time file (skipped during retries, included on final cleanup).
        """
        extensions = [".tree", ".traj"]
        if include_time:
            extensions.append(".time")
        base_path = simulation_xml.replace(".xml", "")
    ...
    def run_beast2(simulation_xml):
        ...
        If the simulation fails (timeout or error), intermediate files
        ...
        use_package_dir = os.path.exists(maybe_package_dir)
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                status = f"BEAST2 simulation for {simulation_xml} timed out (attempt {attempt})."
            ...
            if success:
                # Write the time log only on success
                with open(simulation_xml.replace(".xml", ".time"), "w") as time_log:
                    time_log.write(f"{total_elapsed_time}\n")
                return status

            # Simulation failed - clean up and retry (if attempts remain)
            if attempt < max_retries:
                print(f"Simulation failed for {simulation_xml}. Cleaning up and retrying...")
                cleanup_intermediate_files(simulation_xml, include_time=False)
            else:
                print(f"Simulation failed for {simulation_xml} after {attempt} attempts. Giving up.")
                # Clean up files from the final failed attempt
                cleanup_intermediate_files(simulation_xml, include_time=True)
