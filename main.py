...
def run_beast2_simulations_parallel(simulation_xml_list, num_jobs, beast_seed_str, max_retries=3):
    def cleanup_intermediate_files(simulation_xml):
        """
        Remove intermediate files generated by a failed simulation.

        This cleans up .tree, .traj, and .time files to ensure a fresh
        state before retrying the simulation.
        """
        extensions = [".tree", ".traj", ".time"]
    ...
    def run_beast2(simulation_xml, seed_str, attempt=1):
        """
        ...
        """
        print(f"Running simulation: {simulation_xml} (attempt {attempt}, seed {seed_str})")
        ...
        if not success and attempt < max_retries:
            ...
            cleanup_intermediate_files(simulation_xml)
            # Increment the seed for the retry attempt
            new_seed = str(int(seed_str) + attempt)
            return run_beast2(simulation_xml, new_seed, attempt + 1)
    ...
    def run_beast2_with_seed(simulation_xml):
        """Wrapper to start run_beast2 with the initial seed."""
        return run_beast2(simulation_xml, beast_seed_str, attempt=1)

    with ThreadPoolExecutor(max_workers=num_jobs) as executor:
        future_to_xml = {
            executor.submit(run_beast2_with_seed, xml): xml for xml in simulation_xml_list
        }
