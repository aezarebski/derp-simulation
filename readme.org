...
#+subauthor: Thomas Williams

This repository contains the code to simulate a database of
phylogenetic trees that will be used in a machine learning project.
...
file. There are some example configurations provided:
...
The simulations are conditioned to stop at a maximum time, or as soon
as either the prevalence reaches 50,000 or the number of sequences
reaches 1,000. Simulations are also conditioned to have more than one
sequence, and for the pathogen not to have gone extinct.

This conditioning is coded through the following attributes in the
remaster XML template.
...
hyperparameters in the configuration YAML, this will sample a random
...
time at which the sampling is a proportion of the total epidemic
duration that has a \(\text{Beta}(2,2)\) distribution. You can see an
example of this in the following figure.

#+caption: Example tree from Psyduck database with the plot showing the corresponding time of when the sampling activated.
#+name: fig:thing
#+attr_org: :width 500px
#+attr_html: :width 400px
[[./out/sim-psyduck/plots/demo-tree-change-times.png]]

#+begin_src python :tangle src/demo-tree-sampling-prop.py
from Bio import Phylo
import h5py
import pickle
import matplotlib.pyplot as plt
...
** Why in limited time sampling is a Beta(2,2) distribution used?

A \(\text{Beta}(2,2)\) distribution is used for the proportion of the
epidemic that occurs without sampling because this prevents the
activation time falling too close to the start or the end of the
epidemic which makes the simulations very time consuming to perform.
While pushing the times away from the edges, it still allows for
relatively extreme values of this time.
...
[[https://doi.org/10.1093/ve/veab052][Douglas et al (2021)]].

Note that because Douglas et al (2021) uses years as their unit of
time, the net becoming uninfectious rate needed to be scaled (to days
