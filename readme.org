...
- [-] Provide a single database file and some helper functions to read
  ...
  + [X] Store the trees and metadata in a single database file.
    ...
    * [X] Write a function that loads all the data from one simulation
      into the database.
  + [-] The simulation should be configured by a plain text file so
    ...
    * [ ] The configuration should include some of the hyperparameters
      of the simulation:
      - [ ] Range of epidemic duration.
      - [ ] Upper limit on the number of change points.
